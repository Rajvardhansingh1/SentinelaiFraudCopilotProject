...
5. **Create the tables**: nothing to run by hand. Every SentinelAI service calls `init_db()` on startup (`proxy/main.py`), which does `Base.metadata.create_all(engine)` — this creates all 6 tables (`call_logs`, `findings`, `test_run_results`, `agent_action_logs`, `security_events`, `baselines`) the first time the proxy boots against the new `DATABASE_URL`, and is a no-op on every boot after that. Just start the proxy once with `DATABASE_URL` pointed at Supabase and the tables appear.
